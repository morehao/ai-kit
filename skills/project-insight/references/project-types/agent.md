...
- agent 类通常为多关注面项目 → `README.md` + 子文档。拆分依据依旧是用「分文件判定」的"是否多个可独立成文、且各自够深的关注面"，不以代码行数硬切；够深才独立成篇，浅则并入 README 小节。
...
- 若 agent 项目自带会话/元数据存储，仍须跑「数据模型主动探测」并按五要素独立成 `data-model.md`（判定由程序化信号决定，不由"是否业务应用"的主观印象跳过）。
- 拆出子文档后必须遵守索引约定：README 独立「子文档索引」章节一行一条列全全部子文档，正文「核心模块导读表」每一行、提到关注面的章节就地给出 `[xxx.md](xxx.md)` 内链；新增即回写。

### 信息密度分配

- README 承载：定位、概览、架构总览（Agent → Memory → Tools → LLM 编排 的整体职责链）、核心模块导读表、总结。
- 子文档承载：记忆系统、工具调用、Agent 循环、多 Agent 编排等转发深关注点；每篇满足通用四要素（核心结构/执行流程/设计决策/依赖）+ `文件:行号` 真源引用，且每篇至少 1 张属于本关注面的核心链路图。
...
- 「值得学什么」落点：Agent 如何做记忆分层的权衡、工具调用的可靠性设计（重试 / 超时 / 权限）、多 Agent 之间的通信与信任边界。
- 常见坑：
...
- 深挖指引：比较不同 agent 项目的记忆 / 上下文管理 / 工具调用方案，提炼可迁移模式；深度不足标 `[WIP]`/`TODO`，不硬凑。
