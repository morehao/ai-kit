...
- **dsh 侧**：`commands-dsh/` **不是 opencode 软链扩展**，用 `./scripts/dsh-install.sh`（软链 skills 到 `$DSH_HOME/skills` + `dsh plugin --profile web add link:<本仓库>/commands-dsh`）注册进 dsh profile。
