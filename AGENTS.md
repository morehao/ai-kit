...
本仓库集中管理个人 opencode 自定义扩展：`commands/`（斜杠指令）+ `skills/`（技能）。所有说明用**简体中文**。

## 部署方式（任何修改前必读）

这些扩展通过**软链接**注册到 `~/.config/opencode/`，本仓是唯一真源：
- `ln -sfn <ai-kit>/skills/* ~/.config/opencode/skills/`
- `ln -sfn <ai-kit>/commands/git ~/.config/opencode/command/git`
...
- `skills/project-insight/` — 开源项目深度解读。`SKILL.md` 很大（约 60KB），含铁律：真源引用 `文件:行号` 必须经 `scripts/verify-references.mjs` 程序化重定位验证，Mermaid 图必须经 `scripts/check-mermaid.mjs` 校验全 `OK`。`[UNVERIFIED]`/`[MERMAID-ERROR]` 一律保留不静默放过。落盘后**不做 git add/commit**。
